...
impl BalancedRandDataManager {
    /// 加载所有保存的数据
    pub fn load_all_data(file_path: &str) -> Result<HashMap<String, BalancedRandData>, Box<dyn std::error::Error>> {
    ...
    /// 保存所有数据
    pub fn save_all_data(all_data: &HashMap<String, BalancedRandData>, 
                         file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// 查找匹配的数据
    pub fn find_matching_data(all_data: &HashMap<String, BalancedRandData>, 
                              data_type: &str, 
                              params: &[&str]) -> Option<BalancedRandData> {
    ...
    /// 从2D配置数据中提取权重字典
    fn get_plane_config_weight_dict(data: &BalancedRandData) -> Result<HashMap<(u32, u32), f64>, Box<dyn std::error::Error>> {
        if data.data_type != "BalancedRandPlane" {
    ...
    /// 从2D配置数据中提取抽取次数字典
    fn get_plane_config_draw_counts_dict(data: &BalancedRandData) -> Result<HashMap<(u32, u32), u32>, Box<dyn std::error::Error>> {
        if data.data_type != "BalancedRandPlane" {
            return Err("数据类型必须是BalancedRandPlane".into());
        }
...
impl BalancedRand {
    ...
    pub fn save_data(&self, file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let data = BalancedRandData {
            ...
            last_updated: Utc::now().timestamp(),  // 保存时间戳
    ...
    pub fn draw(&mut self, auto_save: bool) -> Result<u32, String> {
        ...
        *count += 1;
        ...
        self.last_draw_round.insert(selected_number, self.current_round as i32);
        self.total_draws += 1;
    ...
    pub fn draw_multiple(&mut self, count: u32, auto_save: bool) -> Result<Vec<u32>, String> {
        ...
        if count > self.candidate_pool.len() as u32 {
            return Err(format!("抽取数量不能超过候选池大小({})", self.candidate_pool.len()));
        }
    ...
    fn update_candidate_pool(&mut self) {
        ...
        if self.whitelist_only_mode {
            ...
            // 平均值过滤
            candidates = self.all_numbers.iter()
                .filter(|&&n| {
                    let count = self.draw_counts.get(&n).copied().unwrap_or(0);
            ...
            if self.get_max_draw_count_gap() > self.max_gap_threshold {
                ...
                if !candidates.is_empty() {
                    let new_average: f64 = candidates.iter()
    ...
    fn weighted_random_select(&self, weights: &HashMap<u32, f64>) -> Result<u32, String> {
        ...
        let (numbers, weight_values): (Vec<u32>, Vec<f64>) = weights.iter()
        ...
        match WeightedIndex::new(&weight_values) {
            ...
            Err(_) => {
                // 如果权重有问题，使用均匀随机
                self.candidate_pool.choose(&mut thread_rng())
                    .copied()
    ...
    fn update_probabilities(&mut self) {
        ...
        for (&number, &weight) in &weights {
            self.current_probabilities.insert(number, weight / total_weight);
        }
    ...
    fn get_max_draw_count_gap(&self) -> u32 {
        ...
        let active_draw_counts: Vec<u32> = all_active.iter()
            .map(|&n| self.draw_counts.get(&n).copied().unwrap_or(0))
            .collect();
...
impl BalancedRandPlane {
    ...
    pub fn new(
        ...
        let param_strings: Vec<&str> = params.iter().map(|s| s.as_str()).collect();
        let data_id_plane = BalancedRandDataManager::generate_id("BalancedRandPlane", &param_strings);
    ...
    /// 批量抽取多个位置
    pub fn draw_multiple_positions(&mut self, count: u32, auto_save: bool) -> Result<Vec<(u32, u32)>, String> {
        let numbers = self.balanced_rand.draw_multiple(count, auto_save)?;
        
        let positions: Vec<(u32, u32)> = numbers.iter()
            .map(|&n| {
        ...
        for row in 1..=self.rows {
            for col in 1..=self.cols {
                ...
                let prob = if is_blacklisted {
                    ...
                    // 获取实际概率
                    self.balanced_rand.current_probabilities
                        .get(&number)
                        .copied()
                        .unwrap_or(0.0)
                };
        ...
        for row in 1..=self.rows {
            for col in 1..=self.cols {
                ...
                let count = if is_blacklisted {
                    ...
                    // 获取实际抽取次数
                    self.balanced_rand.draw_counts
                        .get(&number)
                        .copied()
                        .unwrap_or(0)
                };
        ...
        for row in 1..=self.rows {
            for col in 1..=self.cols {
                ...
                let (draw_count, probability, last_draw_round) = if is_blacklisted {
                    ...
                    // 获取实际数据
                    let draw_count = self.balanced_rand.draw_counts
                        .get(&number)
                        .copied()
                        .unwrap_or(0);
                    
                    let probability = self.balanced_rand.current_probabilities
                        .get(&number)
                        .copied()
                        .unwrap_or(0.0);
                    
                    let last_draw_round = self.balanced_rand.last_draw_round
                        .get(&number)
                        .copied()
                        .unwrap_or(-1);
    ...
    pub fn set_blacklist_positions(&mut self, positions: &[(u32, u32)]) {
        let numbers: Vec<u32> = positions.iter()
...
fn main() {
    ...
    for ((row, col), (count, prob, last_round)) in &stats {
        println!("  - ({}, {}): 抽取次数={}, 概率={:.3}, 最后抽取轮次={}", 
                row, col, count, prob, last_round);
    }
    
    // 检查黑名单位置
    println!("\n黑名单检查:");
    println!("  - (1,1) 是否在黑名单中: {}", plane.is_position_in_blacklist(1, 1));
    println!("  - (2,3) 是否在黑名单中: {}", plane.is_position_in_blacklist(2, 3));
    println!("  - (3,4) 是否在黑名单中: {}", plane.is_position_in_blacklist(3, 4));
